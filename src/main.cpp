...
//                                    line loop at 16bpp.
//                    2026-09-02 -KL  Background image support. A 480x320 RGB565 array in
//                                    flash (src/bg_image.h, generated by tools/png_to_h.py)
//                                    is now the backdrop everywhere: header, labels,
//                                    message band and the cleared part of each bar all
//                                    blit from it instead of filling C_BG, and text is
...
// Background image, generated by tools/png_to_h.py from a 480x320 PNG. The
// __has_include guard keeps the project building before that header has been
// generated; without it every background blit falls back to a flat C_BG fill
// and the screen looks exactly as it did before.
...
void bgToPanel(int16_t x, int16_t y, int16_t w, int16_t h) {
  ...
  tft.startWrite();
  if (x == 0 && w == SCREEN_W) {
    tft.pushImage(0, y, SCREEN_W, h, &BG_IMAGE[(uint32_t)y * SCREEN_W]);
  } else {
    for (int16_t r = 0; r < h; r++)
      tft.pushImage(x, y + r, w, 1, &BG_IMAGE[(uint32_t)(y + r) * SCREEN_W + x]);
  }
  tft.endWrite();
...
// Same, but into the bar band sprite. Coordinates are band-relative, so this
// offsets into the image by BAR_BAND_Y. RAM to RAM, no panel traffic: the
// sprite keeps swapBytes false so each row is a straight memcpy.
static void bgToBand(int16_t x, int16_t y, int16_t w, int16_t h) {
#if HAVE_BG
  if (x == 0 && w == SCREEN_W) {
    barBand.pushImage(0, y, SCREEN_W, h,
                      &BG_IMAGE[(uint32_t)(BAR_BAND_Y + y) * SCREEN_W]);
  } else {
    for (int16_t r = 0; r < h; r++)
      barBand.pushImage(x, y + r, w, 1,
                        &BG_IMAGE[(uint32_t)(BAR_BAND_Y + y + r) * SCREEN_W + x]);
  }
#else
  barBand.fillRect(x, y, w, h, C_BG);
#endif
}


void drawHeader() {
  bgToPanel(0, HDR_Y, SCREEN_W, HDR_H);
  tft.setTextColor(C_GREEN);   // single argument = transparent, keeps the image
...
void drawLabels() {
  bgToPanel(0, LBL_Y, SCREEN_W, LBL_H);
  tft.setTextDatum(MC_DATUM);
  for (uint8_t i = 0; i < 4; i++) {
    int16_t cx = i * CELL_W + CELL_W / 2;
    tft.setTextColor(C_GREEN);   // transparent, so the image shows behind
...
void drawBar(uint8_t i, bool force = false) {
  ...
  if (!barBandOk) {
    ...
    bgToPanel(bx + 2 + fill, BAR_BAND_Y + BAR_FILL_Y,
              BAR_INNER - 4 - fill, BAR_FILL_H);
    return;
  }

  // Compose into the canvas. No panel traffic in this part.
  barBand.fillRect(bx + 2, BAR_FILL_Y, fill, BAR_FILL_H, C_GREEN);
  bgToBand(bx + 2 + fill, BAR_FILL_Y, BAR_INNER - 4 - fill, BAR_FILL_H);
...
void drawMessage() {
  bgToPanel(0, MSG_Y, SCREEN_W, MSG_H);
  if (!msgShown) return;
  int16_t cy = MSG_Y + MSG_H / 2;
  tft.drawRect(20, MSG_Y + 12, SCREEN_W - 40, MSG_H - 24, C_GREEN);
  tft.setTextDatum(MC_DATUM);
  tft.setTextColor(C_GREEN);   // transparent over the background image
...
void drawScreen() {
  bgToPanel(0, 0, SCREEN_W, SCREEN_H);
  drawHeader();
